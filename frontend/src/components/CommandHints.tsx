...
export default function CommandHints({ prefix, onPick }: CommandHintsProps) {
  ...
  return (
    <div className="mb-2 overflow-hidden rounded-lg border border-slate-700 bg-slate-900 shadow-xl">
      {matches.map((c) => (
        <button
          key={c.name}
          type="button"
          onClick={() => onPick(c.name)}
          className="flex w-full items-center justify-between gap-4 px-3 py-2 text-left text-sm hover:bg-slate-800"
        >
