...
| **Tested where it counts** | 253 backend tests against real PostgreSQL, Redis and Elasticsearch (Testcontainers), ArchUnit boundary rules, PITest mutation testing on the security-critical classes, and 21 Playwright end-to-end scenarios |
...
- **Refresh tokens** with rotation, IP/User-Agent metadata tracking, and server-side revocation — short-lived access tokens are renewed transparently
- **Revocable access tokens** — a JWT cannot normally be taken back, so signing out, banning an account, revoking a device, resetting or changing a password would each leave the access token working until it expired. A per-user "tokens issued before this instant are void" watermark in Redis is checked on every request and on the WebSocket handshake, so ending a session ends it now rather than within the hour
...
- **Private channels** — invite-only: joining by id works for public channels, and a private channel (which is also what every DM and group DM is) gains members only when an owner or moderator adds them. Reads, live subscriptions and the member roster are all gated on membership, so the channel id is never the thing standing between an outsider and the conversation
...
> **Deploying this as something other than a demo?** Two defaults exist to make the hosted demo work and should be turned off: `APP_DEMO_SEED=false` (otherwise a `demo` account is created with the published password below, and given a deliberately generous login throttle) and `SWAGGER_ENABLED=false` (stops publishing the API map). Set `APP_ENCRYPTION_SECRET` before ever rotating `JWT_SECRET` — they are the same value by default, and rotating it would make stored 2FA secrets undecryptable.
...
|---|---|---|
| **Backend integration** (JUnit 5 + Testcontainers) | real PostgreSQL, Redis and Elasticsearch containers — auth, 2FA, token revocation, channel authorisation, messaging, search and its retry path, webhooks, admin, the outbox, and a live STOMP round-trip | **253 tests · 75% line coverage** (JaCoCo) |
